...
PermissionSchema.methods.evaluateCondition = function(key: string, condition: any, context: any): boolean {
...
PermissionSchema.methods.getContextValue = function(keyPath: string, context: any): any {
...
PermissionSchema.methods.evaluateOperator = function(operator: string, contextValue: any, conditionValue: any): boolean {
  switch (operator) {
    case 'equals':
...
PermissionSchema.statics.createSystemPermission = function(name: string, resource: string, action: string, description: string, conditions?: any) {
  return this.create({
    name,
    resource,
    action,
...
PermissionSchema.pre('save', function(next) {
  if (this.isModified('name')) {
    ...
    if (!namePattern.test(this.name)) {
      const error = new Error('Permission name must follow pattern: resource:action:description');
      return next(error);
    }
  }
  next();
