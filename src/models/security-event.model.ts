...
export interface SecurityEventDocument extends Omit<SecurityEvent, '_id'>, Document {
...
interface SecurityEventModelStatics extends mongoose.Model<SecurityEventDocument> {
  findCriticalEvents(): Promise<SecurityEventDocument[]>;
  findUnresolvedEvents(severity?: SecurityEventSeverity): mongoose.Query<SecurityEventDocument[], SecurityEventDocument>;
  getEventStats(timeRange?: { start: Date; end: Date }): Promise<any>;
  createEvent(
    type: SecurityEventType,
    description: string,
    ipAddress: string,
