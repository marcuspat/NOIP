...
RoleSchema.methods.addPermission = async function(permission: Permission | string): Promise<void> {
...
RoleSchema.methods.removePermission = async function(permission: Permission | string): Promise<void> {
  const permissionId = typeof permission === 'string' ? permission : permission._id;

  this.permissions = this.permissions.filter((id: string) => id.toString() !== permissionId.toString());
  await this.save();
};

// Method to check if role has specific permission
RoleSchema.methods.hasPermission = function(permissionName: string): boolean {
  return this.permissions.some((permission: any) => permission.name === permissionName);
};

// Method to get inherited permissions from parent roles
RoleSchema.methods.getInheritedPermissions = async function(): Promise<Permission[]> {
  ...
  if (this.parentRoles && this.parentRoles.length > 0) {
    for (const parentRoleId of this.parentRoles) {
      const parentRole = await mongoose.model('Role').findById(parentRoleId).populate('permissions');
      if (parentRole) {
        inheritedPermissions.push(...parentRole.permissions);
        // Recursively get permissions from parent's parents
        const parentInherited = await (parentRole as RoleDocument).getInheritedPermissions();
...
RoleSchema.statics.createSystemRole = function(name: string, description: string, permissions: string[] = []) {
  return this.create({
    name,
    description,
    permissions,
...
RoleSchema.pre('save', async function(next) {
  ...
  if (this.isModified('parentRoles') && this.parentRoles && this.parentRoles.length > 0) {
    const hasCircularDependency = await this.checkCircularDependency(this._id, this.parentRoles);
...
RoleSchema.methods.checkCircularDependency = async function(roleId: string, parentIds: string[]): Promise<boolean> {
