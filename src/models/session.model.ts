...
SessionSchema.methods.extendSession = async function(duration: number = 30 * 60 * 1000): Promise<void> {
...
SessionSchema.methods.addSecurityEvent = async function(event: string, details: any = {}): Promise<void> {
...
SessionSchema.statics.findByDeviceFingerprint = function(deviceFingerprint: string) {
  return this.find({
    deviceFingerprint,
    isActive: true,
    expiresAt: { $gt: new Date() }
...
SessionSchema.statics.revokeAllByUser = async function(userId: string): Promise<void> {
...
SessionSchema.statics.revokeAllExcept = async function(userId: string, currentSessionId: string): Promise<void> {
  await this.updateMany(
...
SessionSchema.statics.cleanupExpired = async function(): Promise<number> {
  const result = await this.deleteMany({
    $or: [
      ...
      { isActive: false, lastActivity: { $lt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } } // Inactive for 7 days
    ]
  });
...
SessionSchema.statics.getStatistics = async function() {
  ...
  return {
    ...
    active: await this.countDocuments({ isActive: true, expiresAt: { $gt: now } }),
    today: await this.countDocuments({ createdAt: { $gte: dayAgo } }),
    thisWeek: await this.countDocuments({ createdAt: { $gte: weekAgo } }),
    mfaVerified: await this.countDocuments({ mfaVerified: true, isActive: true }),
