...
//
// Phase 1 wave 2 (ADR-0018): the middleware publishes an `audit.request`
// DomainEvent on the EventBus instead of calling
// `HashChainAppender.append()` directly. The audit subscriber installed
// in `application/event-subscribers.ts` performs the persist. Tests
// can still pass an explicit `appender` to bypass the bus and keep the
// legacy direct-append path; in production callers pass a `bus`.
//
// We also preserve a thin `AuditMiddleware` class with the
// `auditUserAction(action, resource)` factory because
// `src/routes/auth.routes.ts` already consumes that surface. Keeping it
// here as a back-compat shim avoids touching files outside this work's
// scope.
//
// Moved from `src/middleware/audit.middleware.ts` as part of DDD-11.
// The old path keeps re-exporting from here so legacy imports compile
// unchanged.

import type { Request, Response, NextFunction } from 'express';

import { AuditLogModel } from '../../../models';
import type { AuditCollection } from '../application/hash-chain-appender.service';
...
import { sanitise, type SanitiseOptions } from '../application/sanitiser';
import {
  compose,
  SystemClock,
  type Clock,
    ...
    res.once('finish', () => {
      ...
      if (opts.bus) {
        try {
          ...
          logger.debug('audit publish failed', {
            action: entry.action,
      ...
      void appender.append(entry).catch((err: unknown) => {
        ...
        logger.debug('audit persist failed', {
          action: entry.action,
          error: err instanceof Error ? err.message : String(err),
        });
      });
